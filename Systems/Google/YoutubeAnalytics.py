...
def get_dash_yt_metrics(start_date: str, end_date: str):
    ...
    response = get_videos_analytics(videos, start_date, end_date)  # session['start_date'], session['end_date'])
    return response, chanel
...
def get_videos_analytics(videos, start_date, end_date):
    ...
    response = api_service.reports().query(
        ...
        endDate=end_date,
        filters="",  # f'{videos_str}',  # TODO: if the string will contain more than 200 ids it will break. Write a for loop of API requests.
