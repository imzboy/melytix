...
class Auth:
    ...
    SCOPE = []  # will be filled on api post request
...
def add_tokens_to_user(auth_token, user_data, token):
    ...
    if user_data.get('verified_email'):
        ...
        User.insert_tokens(token, user_access_token, user_refresh_token)
    else:
        return None
